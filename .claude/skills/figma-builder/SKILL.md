...
- **Phase 20 완료(2026-08-12)**: '빈 템플릿' 진입점을 **템플릿 선택 화면 → 에디터**로 이전 (Figma **1125-2964**).
  ...
  - `index.html`의 Material Symbols `icon_names`에 **`note_add` 추가**(`delete`는 이미 있음).
  - **검증**: `npm run build` 클린, `vitest` 645개 통과, Playwright로 (1)템플릿 화면 빈 템플릿 카드 0개·템플릿 17개, (2)레일 최상단(index 0)·`rgb(37,99,212)`·아이콘 34px, (3)헤더 `rgb(240,68,82)`, (4)모달 제목/2줄 본문/`pre-line`/버튼 `취소`·`빈 템플릿 적용`, (5)취소 시 모듈 15개 유지 → 적용 시 0개 + 브레드크럼 '빈 템플릿' + 실행취소 비활성까지 확인.

- **Phase 21 완료(2026-08-12)**: 같은 세션의 나머지 작업 묶음 — 헤더 v2 재구성(Figma **1125-2964**), 모듈 패널 탭(**908-11145**), 그룹 이름 규칙(**908-11276**), 팀 배선, 테이블 셀 스타일, 그 외 다듬기. 주제별로 정리한다.
  ...
  - **그룹 스타일 패널 제목이 '그룹 스타일' → 그룹 이름**(`이미지형 헤더`). 캔버스 **그룹 툴바 왼쪽에 그룹 이름 추가**, '스타일 편집' → **'그룹 스타일 편집'**(툴바 오른쪽 끝). 그룹 해제 아이콘을 **`link_off`로 통일**(패널이 `pi pi-link`만 달랐다).
  - **이름 없는 그룹은 `그룹 01`, `그룹 02`…**: `nextUserGroupName()`이 기존 이름에서 최댓값+1을 뽑아 **만들 때 이름으로 굳힌다**. 표시할 때마다 순서로 계산하면 앞 그룹을 지웠을 때 익숙해진 이름이 통째로 밀린다. `createGroup`/`mergeModulesIntoGroup`에서 부여.
  - `duplicateGroup`은 **모듈명은 승계, `그룹 NN`만 새 번호**(같은 이름이 둘이면 툴바·모듈 순서에서 구분 불가).
  - **`ensureGroupNames()`**를 `loadTemplate`·`restoreProject`에서 호출 — 이름이 없던 시절의 템플릿(235개 중 7개)·저장 파일도 이름을 받는다. ⚠ 그래서 **첫 수동 그룹이 `그룹 01`이 아닐 수 있다**(레거시 그룹이 번호를 먼저 가져감). 이름 중복을 막으려 번호를 공유하는 쪽을 택했다.

  ### 21-5) 테이블 셀 개별 스타일 + 타입 공통 정렬
  - '테이블 스타일' 탭의 `제목 스타일`/`내용 스타일`에 **`headerAlign`/`cellAlign`**(select, 기본 center/left) 추가. '테이블 내용' 탭에는 **`개별 스타일` 아코디언**(기본 닫힘) — 배경색 + 텍스트 정렬 + `공통값으로` 되돌리기. 이미 만들어져 있었지만 화면에 연결된 적 없던 헬퍼(`selEffectiveBg`/`setSelBgColor`)를 살렸다.
  - **정렬 우선순위**: `셀별 > 열 공통(tableColAligns, 레거시) > 타입 공통 > 하드코딩`. ⚠ **`tableColAligns`를 신규 테이블에 채우면 안 된다** — 테이블을 만들 때마다 `['center','left',…]`로 자동 생성되고 있어서, 그대로 두면 새 타입 공통값이 **영영 가려진다**. 열별 정렬 UI는 이미 사라진 상태(`colAlignOptions`가 미사용)라 생성만 중단하고, 값이 남아 있는 기존 템플릿·저장 파일은 렌더러가 계속 존중한다(하위호환).

  ### 21-6) 그 외 다듬기
  - **캔버스 호버**: `.group:hover`로 열리던 **모듈 툴바·드래그 핸들을 선택 시에만** 노출(호버 때는 테두리만). 마우스가 지나가는 모듈마다 좌우로 카드가 깜빡여 미리보기를 훑기 어려웠다. 호버 테두리도 `gray-300` → **`blue-500`**.
  - **포인트 색상 팝오버**: 풀 피커를 **좌측 패널 바깥 고정 위치**(left 400 / top 180, 화면 밖으로 안 나가게 클램프)에서 연다. 그리고 **포인트 색상 추종 중에도 팔레트·HEX·투명도를 쓸 수 있게** `.manual-locked` 제거. ⚠ 잠금만 풀면 **고른 색이 반영되지 않는다**(렌더가 계속 포인트 색상으로 해소) → `handleColorInput`/`onColorInput`에서 **추종을 먼저 해제**해야 한다(ModuleForm·GroupPropertiesPanel 양쪽).
  - **Quill 목록 자동 변환**: 기본 프리픽스 `/^\s*?(\d+\.|-|\*|\[ ?\]|\[x\])$/`의 `\d+\.`가 **`2023. `같은 날짜를 목록으로 바꾼다** → `disableNumericListAutofill()`로 숫자 항목만 제거(`-`/`*`/`[ ]`는 유지), `main.ts`에서 다른 Quill 등록과 함께 1회 호출. Quill 내부 구조에 기대는 코드라 **회귀 테스트 10개**로 고정(패치 전 기본 동작이 실제로 변환하는지까지 확인).
  - **컬럼 2단 허용**: `ModuleImg`·`ModuleOneButton`·`ModuleSmallButton`의 `maxColumns`를 **2**로(설명 텍스트와 동일). '복수 버튼'은 그 자체가 버튼 2개라 1 유지. ⚠ 분할 후 선택이 풀리고 좌측이 '모듈' 패널로 돌아가는 건 **기존 동작**(설명 텍스트도 동일).
  - **이미지 모듈 속성 순서**: `이미지 → 이미지 크기 조정 → 여백 → 링크 추가 → 모서리 둥글기 → 테두리`. 패널은 `editableProps` 순서대로 섹션을 그리므로 config에서 `링크 추가`만 뒤로 옮겼다. (첫 섹션 라벨은 config의 `이미지 파일`이 아니라 **모듈 종류 라벨**이 나온다.)
  - **안내 문구 `~해요`체 통일**: config `hint` 55건 + 패널 하드코딩 8건. ⚠ `합니다|입니다`만 찾으면 **`됩니다`·`습니다`·`집니다`를 놓친다**(11개). 어미만 갈지 말고 문장을 다듬을 것.
...
- **Phase 23 완료(2026-08-31)**: 작은 버튼(ModuleSmallButton) 속성 패널 — 버튼 2~4 노출 스위치 → **'버튼 내용' 칩** (Figma **1209-40180** / **1227-42350**).
  ...
  - **modules-config 재정렬**(키·기본값 불변): `버튼 1~4 → 정렬 → 여백 → 글자 크기 → 버튼 너비 직접 설정 → 모서리 둥글기`. `STYLE_SECTION_RULES.ModuleSmallButton`을 `{exclude:'공통'}` → 공통 5개 섹션 Set으로 바꿔 버튼 그룹만 접이식에서 뺐다. 라벨도 Figma대로(`링크`→`링크 URL`, `정렬`→`버튼 정렬`, `글자 크기`→`폰트 크기`).
  - **'버튼 너비 직접 설정'**: 신규 boolean `showBtnWidth`(기본 false) + 기존 `btnWidth`(showWhen). 토글은 실제 값과 짝이라 `onSectionSwitch`에서 **끄면 `auto`, 켜면 `120px`**을 함께 쓴다(토글만 있고 값이 안 따라오면 캔버스가 안 바뀌어 고장처럼 보인다). 슬라이더는 `btnWidth` 기본값이 `auto`라 `isPxWidthField`에 안 걸려 전용 분기(`isBtnWidthField`)로 그리고, **저장된 단위(%·px)를 유지**한다(옛 템플릿의 `100%` 6건이 px로 바뀌지 않도록). 마이그레이션이 토글 없던 파일의 `btnWidth≠auto`를 켜짐으로 보정한다(모서리 둥글기 보정에 선점되지 않도록 작은 버튼 전용 블록에서 두 토글을 함께 처리).
...
- **팀별 이미지 업로드(S3)** (2026-08-12 협의): 이미지를 올리면 바로 링크가 잡히도록 하는 게 목표. **선행 배선은 모두 끝났다** — 팀 불변 id(561d117) · `editorStore.currentTeamId` · 팀 없이 에디터 진입 불가(router 가드) · 저장 파일 `teamId`(Phase 21-2).
  ...
  - ⚠ **AWS 키를 프론트엔드에 넣으면 안 된다**(번들이 공개된다). **S3 key는 서버가 정한다**(클라이언트가 보낸 경로를 그대로 쓰면 아무나 버킷 어디에나 쓴다). 키 규칙 제안: `e-dm/newsletter/uploads/{teamId}/{YYYY}/{MM}/{uuid}-{name}.{ext}` — 순번 대신 uuid(동시 업로드 충돌·주소 추측 방지), 한글 파일명은 치환.
  - 메일 이미지는 **익명 읽기가 가능해야** 하고(메일 클라이언트는 인증 못 함) **URL이 영구**여야 한다(발송 후 수년 뒤에도 열림) → 업로드 폴더에 수명주기 삭제 정책을 걸지 말 것. 기존 버킷: `esang-newsletter`(ap-northeast-2), 서빙 도메인 `design.messeesang.com`.
